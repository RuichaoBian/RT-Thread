#line 1 "..\\..\\components\\drivers\\usb\\usbdevice\\core\\core.c"
...
#line 30 "..\\..\\components\\drivers\\usb\\usbdevice\\core\\core.c"
...
#line 31 "..\\..\\components\\drivers\\usb\\usbdevice\\core\\core.c"
#line 32 "..\\..\\components\\drivers\\usb\\usbdevice\\core\\core.c"
...
static rt_err_t _get_interface(struct udevice* device, ureq_t setup)
{
    ...
    intf = rt_usbd_find_interface(device, setup->index & 0xFF, (0));
...
static rt_err_t _set_interface(struct udevice* device, ureq_t setup)
{
    ...
    {
        ep = (uep_t)((struct uendpoint *)((char *)(i) - (unsigned long)(&((struct uendpoint *)0)->list)));
...
static rt_err_t _set_config(struct udevice* device, ureq_t setup)
{
    ...
    for (i=cfg->func_list.next; i!=&cfg->func_list; i=i->next)
    {
        ...
        ufunction_t func = (ufunction_t)((struct ufunction *)((char *)(i) - (unsigned long)(&((struct ufunction *)0)->list)));
        for(j=func->intf_list.next; j!=&func->intf_list; j=j->next)
        {
            intf = (uintf_t)((struct uinterface *)((char *)(j) - (unsigned long)(&((struct uinterface *)0)->list)));
            setting = intf->curr_setting;
            for(k=setting->ep_list.next; k != &setting->ep_list; k=k->next)
            {
                ep = (uep_t)((struct uendpoint *)((char *)(k) - (unsigned long)(&((struct uendpoint *)0)->list)));
        ...
        do{ if(func->ops->enable != (0) && func->enabled == 0) { if(func->ops->enable(func) == 0) func->enabled = 1; } }while(0);
    }
...
static rt_err_t _standard_request(struct udevice* device, ureq_t setup)
{
    ...
    case 0x02:
        ...
        case 0x00:
        {
            ...
            ep = rt_usbd_find_endpoint(device, (0), setup->index);
            value = ep->stalled;        
            rt_usbd_ep0_write(device, &value, 2);
        }
        break;
        case 0x01:
        {
            ...
            if(0 == setup->value && ep->stalled == 1)
            {
                ...
                {
                    req = (uio_request_t)((struct uio_request *)((char *)(node) - (unsigned long)(&((struct uio_request *)0)->list)));                    
...
static rt_err_t _setup_request(udevice_t device, ureq_t setup)
{
    ...
    switch((setup->request_type & 0x60))
    {
    case 0x00:
...
static rt_err_t _data_notify(udevice_t device, struct ep_msg* ep_msg)
{
    ...
    if(ep->ep_desc->bEndpointAddress & 0x80)
    {
        if(ep->request.remain_size >= ep->ep_desc->wMaxPacketSize)
        {
            dcd_ep_write(device->dcd, ep->ep_desc->bEndpointAddress,
                ep->request.buffer, ep->ep_desc->wMaxPacketSize);
            ep->request.remain_size -= ep->ep_desc->wMaxPacketSize;
            ep->request.buffer += ep->ep_desc->wMaxPacketSize;     
        }
    ...
    else
    {
        ...
        if(size == 0)
        {
            size = dcd_ep_read(device->dcd, ep->ep_desc->bEndpointAddress,
                ep->request.buffer);
        }
        ...
        {
            ; ep->handler(func, ep->request . size);
        }
    }
...
static rt_err_t _ep0_out_notify(udevice_t device, struct ep_msg* ep_msg)
{
    ...
    if(size == 0)
    {
        size = dcd_ep_read(device->dcd, 0x00, ep0->request.buffer);
        if(size == 0)
        {
            return 0;
        }        
...
static rt_err_t _sof_notify(udevice_t device)
{
    ...
    {
        func = (ufunction_t)((struct ufunction *)((char *)(i) - (unsigned long)(&((struct ufunction *)0)->list)));
        if(func->ops->sof_handler != (0))
...
static rt_err_t _stop_notify(udevice_t device)
{
    ...
    {
        func = (ufunction_t)((struct ufunction *)((char *)(i) - (unsigned long)(&((struct ufunction *)0)->list)));
        do{ if(func->ops->disable != (0) && func->enabled == 1) { func->enabled = 0; func->ops->disable(func); } }while(0);
    }
...
static rt_size_t rt_usbd_ep_write(udevice_t device, uep_t ep, void *buffer, rt_size_t size)
{
    ...
    maxpacket = ep->ep_desc->wMaxPacketSize;
    if(ep->request.remain_size >= maxpacket)
    {
        sent_size = dcd_ep_write(device->dcd, ep->ep_desc->bEndpointAddress, ep->request.buffer, maxpacket);
        ep->request.remain_size -= sent_size;
        ep->request.buffer += maxpacket;    
    }
    else
    {
        sent_size = dcd_ep_write(device->dcd, ep->ep_desc->bEndpointAddress, ep->request.buffer, 
...
static rt_size_t rt_usbd_ep_read_prepare(udevice_t device, uep_t ep, void *buffer, rt_size_t size)
{
    ...
    return dcd_ep_read_prepare(device->dcd, ep->ep_desc->bEndpointAddress, buffer, size);
}
...
rt_err_t rt_usbd_device_set_descriptor(udevice_t device, udev_desc_t dev_desc)
{
    ...
    rt_memcpy((void *)&device->dev_desc, (void *)dev_desc, 0x12);
...
uconfig_t rt_usbd_config_new(void)
{
    ...
    cfg->cfg_desc.bLength = 0x9;
    cfg->cfg_desc.type = 0x02;
    cfg->cfg_desc.wTotalLength = 0x9;
...
udevice_t rt_usbd_find_device(udcd_t dcd)
{
    ...
    {
        device = (udevice_t)((struct udevice *)((char *)(node) - (unsigned long)(&((struct udevice *)0)->list)));
...
uconfig_t rt_usbd_find_config(udevice_t device, rt_uint8_t value)
{
    ...
    {
        cfg = (uconfig_t)((struct udevice *)((char *)(node) - (unsigned long)(&((struct udevice *)0)->list)));
...
ualtsetting_t rt_usbd_find_altsetting(uintf_t intf, rt_uint8_t value)
{
    ...
    {
        setting =(ualtsetting_t)((struct ualtsetting *)((char *)(i) - (unsigned long)(&((struct ualtsetting *)0)->list)));
...
rt_err_t rt_usbd_device_add_config(udevice_t device, uconfig_t cfg)
{
    ...
    for (i=cfg->func_list.next; i!=&cfg->func_list; i=i->next)
    {
        ...
        for(j=func->intf_list.next; j!=&func->intf_list; j=j->next)
        {
            ...
            {
                ep = (uep_t)((struct uendpoint *)((char *)(k) - (unsigned long)(&((struct uendpoint *)0)->list)));
                if(rt_usbd_ep_assign(device, ep) != 0)
                {
            ...
            rt_memcpy((void*)&cfg->cfg_desc.data[cfg->cfg_desc.wTotalLength - 0x9], 
...
rt_err_t rt_usbd_ep_set_stall(udevice_t device, uep_t ep)
{
    ...
    ret = dcd_ep_set_stall(device->dcd, ep->ep_desc->bEndpointAddress);
    if(ret == 0)
    {
        ep->stalled = 1;
    }
...
rt_err_t rt_usbd_ep_clear_stall(udevice_t device, uep_t ep)
{
    ...
    ret = dcd_ep_clear_stall(device->dcd, ep->ep_desc->bEndpointAddress);
    if(ret == 0)
    {
        ep->stalled = 0;
    }
...
rt_err_t rt_usbd_ep0_setup_handler(udcd_t dcd, struct urequest* setup)
{
    ...
    if(setup == (0))
    {
        size = dcd_ep_read(dcd, 0x00, (void*)&msg.content.setup);
...
rt_err_t rt_usbd_ep0_in_handler(udcd_t dcd)
{
    ...
    {
        dcd_ep_write(dcd, 0x80, dcd->ep0.request.buffer, dcd->ep0.id->maxpacket);
        dcd->ep0.request.remain_size -= dcd->ep0.id->maxpacket;
    }
    else if(dcd->ep0.request.remain_size > 0)
    {
        dcd_ep_write(dcd, 0x80, dcd->ep0.request.buffer, dcd->ep0.request.remain_size);
        dcd->ep0.request.remain_size = 0;
    }
    else
    {
        dcd_ep_write(dcd, 0x80, (0), 0);
    }
...
rt_size_t rt_usbd_ep0_write(udevice_t device, void *buffer, rt_size_t size)
{
    ...
    {
        sent_size = dcd_ep_write(device->dcd, 0x80, ep0->request.buffer, ep0->id->maxpacket);
        ep0->request.remain_size -= sent_size;
        ep0->request.buffer += ep0->id->maxpacket;
    }
    else
    {
        sent_size = dcd_ep_write(device->dcd, 0x80, ep0->request.buffer, ep0->request.remain_size);
        ep0->request.remain_size -= sent_size;        
    }
    ...
    rt_err_t (*rx_ind)(udevice_t device, rt_size_t size))
{
    ...
    dcd_ep_read_prepare(device->dcd, 0x00, buffer, size);
...
static rt_uint8_t usb_thread_stack[2048];
...
static rt_uint8_t usb_mq_pool[(32+sizeof(void*))*16];
...
rt_err_t rt_usbd_core_init(void)
{
    ...
    rt_mq_init(&usb_mq, "usbd", usb_mq_pool, 32,
            sizeof(usb_mq_pool), 0x00);

     
    rt_thread_init(&usb_thread, "usbd", rt_usbd_thread_entry, (0),
            usb_thread_stack, 2048, 8, 20);
