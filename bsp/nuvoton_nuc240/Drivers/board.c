...
static void rt_hw_system_init(void)
{
    ...
    CLK_EnableXtalRC(CLK_PWRCON_XTL12M_EN_Msk);   //add by RC

    /* Waiting for external XTAL clock ready */
    CLK_WaitClockReady(CLK_CLKSTATUS_XTL12M_STB_Msk);//add by RC

    /* Set core clock as PLL_CLOCK from PLL */
    CLK_SetCoreClock(BOARD_PLL_CLOCK);
		
		CLK_EnableModuleClock(USBD_MODULE);   									//add by RC
		
		CLK_SetModuleClock(USBD_MODULE, 0, CLK_CLKDIV_USB(3)); //add by RC
