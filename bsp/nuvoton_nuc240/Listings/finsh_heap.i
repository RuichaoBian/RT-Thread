#line 1 "..\\..\\components\\finsh\\finsh_heap.c"
...
#line 30 "..\\..\\components\\finsh\\finsh_heap.c"
...
#line 32 "..\\..\\components\\finsh\\finsh_heap.c"

__attribute__((aligned(4)))
u_char finsh_heap[128];
...
int finsh_heap_init(void)
{
    ...
    free_list           = (struct finsh_block_header*)(&finsh_heap[0]);
	free_list->length   = 128 - sizeof(struct finsh_block_header);
    free_list->next     = 0;
...
void* finsh_heap_allocate(size_t size)
{
    ...
    for (header = free_list;
        ((header != 0) && (header->length <= size + sizeof(struct finsh_block_header)));
        header = header->next) ;
	...
	memset((u_char*)((struct finsh_block_header*)header + 1), 0, size);

	return (u_char*)((struct finsh_block_header*)header + 1);
}
...
void  finsh_heap_free(void*ptr)
{
	...
	header = (struct finsh_block_header*)((u_char*)ptr - sizeof(struct finsh_block_header));
...
static void finsh_heap_gc(void)
{
    ...
    for (header = allocate_list; header != 0; )
    {
    	for (i = 0; i < 8; i ++)
    	{
    		if (global_variable[i].type != finsh_type_unknown)
    		{
    			if (global_variable[i].value.ptr == (u_char*)((struct finsh_block_header*)header + 1))
					break;
    		}
    	}
    	...
    	if (i == 8)
    	{
    		finsh_heap_free((u_char*)((struct finsh_block_header*)temp + 1));
    	}
    }
}
...
void finsh_block_insert(struct finsh_block_header** list, struct finsh_block_header* header)
{
    ...
    if (node->next != 0) header->next = node->next;
    node->next      = header;
}
...
void finsh_block_split(struct finsh_block_header* header, size_t size)
{
    ...
    next = (struct finsh_block_header*)((u_char*)header + sizeof(struct finsh_block_header) + size);
