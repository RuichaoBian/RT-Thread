#line 1 "..\\..\\components\\finsh\\finsh_parser.c"
...
#line 31 "..\\..\\components\\finsh\\finsh_parser.c"
...
#line 33 "..\\..\\components\\finsh\\finsh_parser.c"
...
#line 34 "..\\..\\components\\finsh\\finsh_parser.c"
...
#line 35 "..\\..\\components\\finsh\\finsh_parser.c"
#line 1 "..\\..\\components\\finsh\\finsh_parser.h"
...
#line 37 "..\\..\\components\\finsh\\finsh_parser.c"
...
static struct finsh_node* proc_variable_decl(struct finsh_parser* self)
{
	...
	(token) = finsh_token_token(&(self->token));
	switch ( token )
	{
	case finsh_token_type_comma: 
		if (proc_identifier(self, id) == 0)
		{
		...
		if ( token == finsh_token_type_assign )
		{
			...
			if (assign != 0)
			{
				...
				end = make_sys_node(19, idnode, assign);
				node = end;
		...
		while ( token == finsh_token_type_comma )
		{
			...
			if ( token == finsh_token_type_assign )
			{
				...
				if (assign != 0)
				{
					...
					if (node != 0)
					{
						((end)->sibling) = make_sys_node(19, idnode, assign);
						end = ((end)->sibling);
					}
					else
					{
		...
		if ( (token) != (finsh_token_type_semicolon) ) { finsh_error_set(1); (&(self->token))->replay = 1; };
		return node;

	case finsh_token_type_assign: 
	{
		struct finsh_node *idnode;
	...
	case finsh_token_type_semicolon: 
		return node;
...
static enum finsh_type proc_type(struct finsh_parser* self)
{
	...
	(token) = finsh_token_token(&(self->token));
	if ( ((token) == finsh_token_type_void || (token) == finsh_token_type_char || (token) == finsh_token_type_short || (token) == finsh_token_type_int || (token) == finsh_token_type_long) )  
	{
		switch (token)
		{
	...
	else if ( token == finsh_token_type_unsigned )  
	{
		(token) = finsh_token_token(&(self->token));
		if ( ((token) == finsh_token_type_void || (token) == finsh_token_type_char || (token) == finsh_token_type_short || (token) == finsh_token_type_int || (token) == finsh_token_type_long) )
		{
...
static int proc_identifier(struct finsh_parser* self, char* id)
{
	...
	(token) = finsh_token_token(&(self->token)); if ( (token) != (finsh_token_type_identifier) ) { finsh_error_set(1); (&(self->token))->replay = 1; };

	strncpy(id, (char*)self->token.string, 16);
...
static struct finsh_node* proc_expr_statement(struct finsh_parser* self)
{
	...
	if ( token != finsh_token_type_semicolon )
	{
		...
		(token) = finsh_token_token(&(self->token)); if ( (token) != (finsh_token_type_semicolon) ) { finsh_error_set(1); (&(self->token))->replay = 1; };
	}
...
static struct finsh_node* proc_assign_expr(struct finsh_parser* self)
{
	...
	if (token == finsh_token_type_assign)
	{
		...
		return make_sys_node(19, or, assign);
	}
	else (&(self->token))->replay = 1;
...
static struct finsh_node* proc_inclusive_or_expr(struct finsh_parser* self)
{
	...
	while ( token == finsh_token_type_or )
	{
		...
		if (xor_new == 0) finsh_error_set(5);
		else xor = make_sys_node(13, xor, xor_new);
...
static struct finsh_node* proc_exclusive_or_expr(struct finsh_parser* self)
{
	...
	while ( token == finsh_token_type_xor )
	{
		...
		if (and_new == 0) finsh_error_set(5);
		else and = make_sys_node(14, and, and_new);
...
static struct finsh_node* proc_and_expr(struct finsh_parser* self)
{
	...
	while ( token == finsh_token_type_and )
	{
		...
		if (shift_new == 0) finsh_error_set(5);
		else shift = make_sys_node(12, shift, shift_new);
...
static struct finsh_node* proc_shift_expr(struct finsh_parser* self)
{
	...
	while ( token == finsh_token_type_shl || token == finsh_token_type_shr)
	{
		...
		if (add_new == 0) finsh_error_set(5);
		else
		{
			switch (token)
			{
			case finsh_token_type_shl:
				add = make_sys_node(16, add, add_new);
				break;
			case finsh_token_type_shr:
				add = make_sys_node(17, add, add_new);
				break;
			default:
				finsh_error_set(5);
				break;
...
static struct finsh_node* proc_additive_expr(struct finsh_parser* self)
{
	...
	while ( token == finsh_token_type_sub || token == finsh_token_type_add )
	{
		...
		if (mul_new == 0) finsh_error_set(5);
		else
		{
			switch (token)
			{
			case finsh_token_type_sub:
				mul = make_sys_node(8, mul, mul_new);
				break;
			case finsh_token_type_add:
				mul = make_sys_node(7, mul, mul_new);
				break;
			default:
				finsh_error_set(5);
				break;
...
static struct finsh_node* proc_multiplicative_expr(struct finsh_parser* self)
{
		...
		token == finsh_token_type_mod )
	{
		...
		if (cast_new == 0) finsh_error_set(5);
		else
		{
			switch (token)
			{
			case finsh_token_type_mul:
				cast = make_sys_node(9, cast, cast_new);
				break;

			case finsh_token_type_div:
				cast = make_sys_node(10, cast, cast_new);
				break;

			case finsh_token_type_mod:
				cast = make_sys_node(11, cast, cast_new);
				break;
...
static struct finsh_node* proc_cast_expr(struct finsh_parser* self)
{
	...
	{
		type = proc_type(self);
		(token) = finsh_token_token(&(self->token)); if ( (token) != (finsh_token_type_right_paren) ) { finsh_error_set(1); (&(self->token))->replay = 1; };
...
static struct finsh_node* proc_unary_expr(struct finsh_parser* self)
{
	...
	case finsh_token_type_inc:  
		cast = proc_cast_expr(self);
		return make_sys_node(21, cast, 0);

	case finsh_token_type_sub:  
		cast = proc_cast_expr(self);
		return make_sys_node(8, finsh_node_new_long(0), cast);

	case finsh_token_type_dec:  
		cast = proc_cast_expr(self);
		return make_sys_node(22, cast, 0);

	case finsh_token_type_bitwise:  
		cast = proc_cast_expr(self);
		return make_sys_node(15, cast, 0);

    case finsh_token_type_mul:  
        cast = proc_cast_expr(self);
        return make_sys_node(25, cast, 0);

    case finsh_token_type_and:  
        cast = proc_cast_expr(self);
        return make_sys_node(26, cast, 0);
...
static struct finsh_node* proc_postfix_expr(struct finsh_parser* self)
{
		...
		token == finsh_token_type_left_paren )
	{
		...
		{
		case finsh_token_type_inc : 
			postfix = make_sys_node(23, postfix, 0);
			break;

		case finsh_token_type_dec : 
			postfix = make_sys_node(24, postfix, 0);
			break;
		...
		case finsh_token_type_left_paren : 
			{
				...
				postfix = make_sys_node(18, postfix, param_list);
			}
			break;
...
static struct finsh_node* proc_param_list(struct finsh_parser* self)
{
	...
	while (token == finsh_token_type_comma )
	{
		((assign)->sibling) = proc_assign_expr(self);

		if (((assign)->sibling) != 0)	assign = ((assign)->sibling);
		else finsh_error_set(5);
...
void finsh_parser_run(struct finsh_parser* self, const u_char* string)
{
	...
	while (token != finsh_token_type_eof && token != finsh_token_type_bad)
	{
        ...
        case finsh_token_type_identifier:
			...
			if (self->root != 0)
			{
				((node)->sibling) = proc_expr_statement(self);
				if (((node)->sibling) != 0)
					node = ((node)->sibling);
			}
			else
		...
		default:
            if (((token) == finsh_token_type_void || (token) == finsh_token_type_char || (token) == finsh_token_type_short || (token) == finsh_token_type_int || (token) == finsh_token_type_long) || token == finsh_token_type_unsigned)
            {
				...
				if (self->root != 0)
				{
					((node)->sibling) = proc_variable_decl(self);
					if (((node)->sibling) != 0)
						node = ((node)->sibling);
				}
				else
            ...
            else
            {
				...
				if (self->root != 0)
				{
					...
					if (((node)->sibling) != 0)
						node = ((node)->sibling);
					else (token) = finsh_token_token(&(self->token));
				}
				else
				{
					node = proc_expr_statement(self);
