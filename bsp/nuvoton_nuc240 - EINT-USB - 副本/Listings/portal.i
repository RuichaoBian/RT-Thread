#line 1 "..\\..\\components\\drivers\\src\\portal.c"
...
#line 26 "..\\..\\components\\drivers\\src\\portal.c"
...
#line 27 "..\\..\\components\\drivers\\src\\portal.c"
...
static rt_err_t _portal_init(rt_device_t dev)
{
    ...
    if (!(dev)) { rt_assert_handler("dev", __FUNCTION__, 36); };
...
static rt_err_t _portal_open(rt_device_t dev, rt_uint16_t oflag)
{
    ...
    if (!(dev)) { rt_assert_handler("dev", __FUNCTION__, 54); };
    ...
    if (oflag & 0x001)
    {
        err = rt_device_open(portal->read_dev, 0x001);
        if (err != 0)
            return err;
    }
    ...
    if (oflag & 0x002)
    {
        err = rt_device_open(portal->write_dev, 0x002);
        if (err != 0)
            return err;
    }
...
static rt_err_t _portal_close(rt_device_t dev)
{
    ...
    if (!(dev)) { rt_assert_handler("dev", __FUNCTION__, 82); };
...
static rt_size_t _portal_read(rt_device_t dev,
                              ...
                              rt_size_t size)
{
    return rt_device_read((((struct rt_portal_device*)dev)->read_dev),
                          pos, buffer, size);
}
...
static rt_size_t _portal_write(rt_device_t dev,
                               ...
                               rt_size_t size)
{
    return rt_device_write((((struct rt_portal_device*)dev)->write_dev),
                           pos, buffer, size);
}
...
static rt_err_t _portal_rx_indicate(rt_device_t dev, rt_size_t size)
{
    ...
    if (!(dev && dev->type == RT_Device_Class_Pipe)) { rt_assert_handler("dev && dev->type == RT_Device_Class_Pipe", __FUNCTION__, 114); };
...
static rt_err_t _portal_tx_complete(rt_device_t dev, void *buf)
{
    ...
    if (!(dev && dev->type == RT_Device_Class_Pipe)) { rt_assert_handler("dev && dev->type == RT_Device_Class_Pipe", __FUNCTION__, 129); };
                        ...
                        const char *read_dev)
{
    ...
    if (!(portal)) { rt_assert_handler("portal", __FUNCTION__, 177); };
    ...
    portal->parent.control     = (0);

    dev = rt_device_find(write_dev);
    if (dev == (0))
        return -6;
    if (!(dev->type == RT_Device_Class_Pipe)) { rt_assert_handler("dev->type == RT_Device_Class_Pipe", __FUNCTION__, 191); };
    ...
    if (!(dev->type == RT_Device_Class_Pipe)) { rt_assert_handler("dev->type == RT_Device_Class_Pipe", __FUNCTION__, 203); };
    portal->read_dev = dev;
