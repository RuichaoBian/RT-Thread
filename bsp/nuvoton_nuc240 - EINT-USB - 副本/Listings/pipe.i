#line 1 "..\\..\\components\\drivers\\src\\pipe.c"
...
#line 26 "..\\..\\components\\drivers\\src\\pipe.c"
#line 27 "..\\..\\components\\drivers\\src\\pipe.c"
...
#line 28 "..\\..\\components\\drivers\\src\\pipe.c"
...
static void _rt_pipe_resume_writer(struct rt_pipe_device *pipe)
{
    if (!rt_list_isempty(&pipe->suspended_write_list))
    {
        ...
        if (!(pipe ->flag & RT_PIPE_FLAG_BLOCK_WR)) { rt_assert_handler("pipe->flag & RT_PIPE_FLAG_BLOCK_WR", __FUNCTION__, 35); };

         
        thread = ((struct rt_thread *)((char *)(pipe ->suspended_write_list . next) - (unsigned long)(&((struct rt_thread *)0)->tlist)));
                              ...
                              rt_size_t   size)
{
    ...
    if (!(pipe != (0))) { rt_assert_handler("pipe != RT_NULL", __FUNCTION__, 60); };
...
static void _rt_pipe_resume_reader(struct rt_pipe_device *pipe)
{
    ...
    if (!rt_list_isempty(&pipe->suspended_read_list))
    {
        ...
        if (!(pipe ->flag & RT_PIPE_FLAG_BLOCK_RD)) { rt_assert_handler("pipe->flag & RT_PIPE_FLAG_BLOCK_RD", __FUNCTION__, 115); };

         
        thread = ((struct rt_thread *)((char *)(pipe ->suspended_read_list . next) - (unsigned long)(&((struct rt_thread *)0)->tlist)));
                               ...
                               rt_size_t   size)
{
    ...
    pipe = ((struct rt_pipe_device*)(dev));
    if (!(pipe != (0))) { rt_assert_handler("pipe != RT_NULL", __FUNCTION__, 140); };
    ...
    do { rt_base_t level; level = rt_hw_interrupt_disable(); if (rt_interrupt_get_nest() != 0) { rt_kprintf("Function[%s] shall not used in ISR\n", __FUNCTION__); if (!(0)) { rt_assert_handler("0", __FUNCTION__, 164); } } rt_hw_interrupt_enable(level); } while (0);
...
{
    if (cmd == 0x14 && args)
        *(rt_size_t*)args = ((&((struct rt_pipe_device*)(dev))->ringbuffer)->buffer_size - rt_ringbuffer_data_len(&((struct rt_pipe_device*)(dev))->ringbuffer));
    return 0;
}
...
rt_err_t rt_pipe_init(struct rt_pipe_device *pipe,
                      ...
                      rt_size_t size)
{
    if (!(pipe)) { rt_assert_handler("pipe", __FUNCTION__, 216); };
    if (!(buf)) { rt_assert_handler("buf", __FUNCTION__, 217); };
    ...
    return rt_device_register(&(pipe->parent), name, 0x003);
}
;
...
rt_err_t rt_pipe_create(const char *name, enum rt_pipe_flag flag, rt_size_t size)
{
    rt_uint8_t *rb_memptr = (0);
    struct rt_pipe_device *pipe = (0);
