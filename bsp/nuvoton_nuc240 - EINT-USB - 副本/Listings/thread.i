                                ...
                                rt_uint32_t       tick)
{
    ...
    if (!(priority < 32)) { rt_assert_handler("priority < RT_THREAD_PRIORITY_MAX", __FUNCTION__, 158); };
    ...
    thread->cleanup   = 0;
    thread->user_data = 0;
		rt_kprintf("&(thread->thread_timer) = %d",&(thread->thread_timer));
                        ...
                        rt_uint32_t       tick)
{
    ...
    if (!(thread != (0))) { rt_assert_handler("thread != RT_NULL", __FUNCTION__, 218); };
    if (!(stack_start != (0))) { rt_assert_handler("stack_start != RT_NULL", __FUNCTION__, 219); };
...
rt_err_t rt_thread_startup(rt_thread_t thread)
{
    ...
    if (!(thread != (0))) { rt_assert_handler("thread != RT_NULL", __FUNCTION__, 256); };
    if (!(thread->stat == 0x00)) { rt_assert_handler("thread->stat == RT_THREAD_INIT", __FUNCTION__, 257); };
...
rt_err_t rt_thread_detach(rt_thread_t thread)
{
    ...
    if (!(thread != (0))) { rt_assert_handler("thread != RT_NULL", __FUNCTION__, 300); };
...
rt_err_t rt_thread_delete(rt_thread_t thread)
{
    ...
    if (!(thread != (0))) { rt_assert_handler("thread != RT_NULL", __FUNCTION__, 398); };
...
rt_err_t rt_thread_sleep(rt_tick_t tick)
{
    ...
    thread = rt_current_thread;
    if (!(thread != (0))) { rt_assert_handler("thread != RT_NULL", __FUNCTION__, 486); };
...
rt_err_t rt_thread_control(rt_thread_t thread, rt_uint8_t cmd, void *arg)
{
    ...
    if (!(thread != (0))) { rt_assert_handler("thread != RT_NULL", __FUNCTION__, 537); };
...
rt_err_t rt_thread_suspend(rt_thread_t thread)
{
    ...
    if (!(thread != (0))) { rt_assert_handler("thread != RT_NULL", __FUNCTION__, 615); };
...
rt_err_t rt_thread_resume(rt_thread_t thread)
{
    ...
    if (!(thread != (0))) { rt_assert_handler("thread != RT_NULL", __FUNCTION__, 657); };
...
void rt_thread_timeout(void *parameter)
{
    ...
    if (!(thread != (0))) { rt_assert_handler("thread != RT_NULL", __FUNCTION__, 701); };
    if (!(thread->stat == 0x02)) { rt_assert_handler("thread->stat == RT_THREAD_SUSPEND", __FUNCTION__, 702); };
