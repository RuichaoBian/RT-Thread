...
void USBD_IRQHandler(void)
{
    ...
    uint32_t u32State;
		rt_interrupt_enter();
		u32IntSts = USBD_GET_INT_FLAG();   //获取中断状态
		u32State = USBD_GET_BUS_STATE();  //获取USB总线状态
//------------------------------------------------------------------
    if (u32IntSts & USBD_INTSTS_FLDET) {     // USB浮动检测中断状态
        // Floating detect
        USBD_CLR_INT_FLAG(USBD_INTSTS_FLDET);

        if (USBD_IS_ATTACHED()) {    //检测电缆连接状态  返回1 为已连接    0为未链接
            /* USB Plug In */
            USBD_ENABLE_USB();   //使能USB
        }
        else {
            /* USB Un-plug */
            USBD_DISABLE_USB();
...
//------------------------------------------------------------------
    if (u32IntSts & USBD_INTSTS_BUS) {    //USB总线事件中断状态
        /* Clear event flag */
        USBD_CLR_INT_FLAG(USBD_INTSTS_BUS);

        if (u32State & USBD_STATE_USBRST) {   //USB总线复位
            /* Bus reset */
            USBD_ENABLE_USB();
            USBD_SwReset();
        }
        if (u32State & USBD_STATE_SUSPEND) {   //USB总线挂起
            /* Enable USB but disable PHY */
            USBD_DISABLE_PHY();
        }
        if (u32State & USBD_STATE_RESUME) {  //USB总线重启
...
//------------------------------------------------------------------
    if (u32IntSts & USBD_INTSTS_USB) {    //USB事件中断状态
        // USB event
